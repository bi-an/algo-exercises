...
class Success : public Action
{
 public:
  void GetManConclusion(Man *concreteElementA)
  {
    ...
    std::cout << const_cast<char *>(typeid(*concreteElementA).name()) + 6 << " " << const_cast<char *>(typeid(*this).name()) + 6 << "时，背后总有一个伟大的女人"
              << std::endl;
  }
  void GetWomanConclusion(Woman *conreteElementB)
  {
    std::cout << const_cast<char *>(typeid(*conreteElementB).name()) + 6 << " " << const_cast<char *>(typeid(*this).name()) + 6
              << "时，背后总有一个不成功的男人" << std::endl;
  }
};

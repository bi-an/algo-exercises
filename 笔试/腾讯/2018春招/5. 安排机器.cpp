...
bool cmp(Node n1, Node n2) {
	if (n1.time == n2.time) return n1.grade > n2.grade;
...
int main() {
	...
	for (int i = 0, j = 0; i < numTask; i++) { // 遍历任务
		while (j < numMachine && tasks[i].time <= machines[j].time) { // 遍历机器，寻找时间上满足任务条件的机器
			cnt[machines[j].grade]++; // 注意到，machines和tasks的时间都是从大到小排序的，如果某台机器满足前面的任务，那么它一定也满足后面的任务
			j++; // 所以，j不需要（也不能）倒退回去重新遍历机器
		}
		for (int k = tasks[i].grade; k < 101; k++) { // 遍历hashMap cnt，寻找等级上满足任务条件的机器（上面的while循环保证了时间上满足条件）
			if (cnt[k]) {
				cnt[k]--; // 该台机器已经被使用

...
int main() {
  ...
  while(1) {
    rset = allset;
    nready = select(maxfd + 1, &rset, NULL, NULL, NULL); // wait forever, blocked
    if(nready < 0)
    ...
    if(FD_ISSET(listenfd, &rset)) {
      ...
      // inet_ntop convert IPv4 or IPv6 addresses from binary to text form
      ...
      if(connfd > maxfd)
        maxfd = connfd; // parameter for select

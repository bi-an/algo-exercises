...
public:
	...
	bool duplicate(int numbers[], int length, int* duplication) {
		if (!numbers || !duplication || length <= 0) return false;
		for (int i = 0; i < length; i++) {
			if (numbers[i]<0 || numbers[i]>length - 1)
				return false;
		}
		for (int i = 0; i < length; i++) {
			while (numbers[i] != i) {//这里改成if也能在剑指offer上测试通过，但是不正确，如：int numbers[] = { 0,9,1,2,3,4,5,6,8,0 };
				int temp = numbers[i];
...
// 抽屉原则：抽屉从0开始编号，二分搜索抽屉，重复的数必然在数的个数大于抽屉数的区间，再到这个区间去二分查找。

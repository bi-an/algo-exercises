...
public:
	...
	bool operator>(sortObj& obj2) const { return key > obj2.key; }
	bool operator>=(sortObj& obj2) const { return key >= obj2.key; }
	bool operator<(sortObj& obj2) const { return key < obj2.key; }
	bool operator<=(sortObj& obj2) const { return key <= obj2.key; }
	bool operator==(sortObj& obj2) const { return key == obj2.key; }
...
void bubbleSort(int numbers[], int size) {
	if(numbers==nullptr || size<=0) return;
	for (int i = size - 2; i >= 0; i--) {//总比较次数=(size-2)
		for (int j = 0; j <= i; j++) {
...
int Partition(int numbers[], int len, int start, int end) {
	if (numbers == nullptr || len <= 0 || start<0 || end>len || start > end) return -1;
...
void qSort(int numbers[], int size) {
	if (numbers == nullptr || size <= 1) return;
...
void mergeSort(T* arr, int start, int end) {
	if (arr == nullptr || end-start <= 0) return;
	int left = start;
	...
	while (left <= mid && right <= end) {
		if (arr[left] < arr[right]) tmpArr[pos++] = arr[left++]; 
		else tmpArr[pos++] = arr[right++];
	}
	while (left <= mid) tmpArr[pos++] = arr[left++];
	while (right <= end) tmpArr[pos++] = arr[right++];
...
size_t binarySearch(T* arr, size_t len, const T& key){
	...
	while(left<=right){
		...
		if(arr[mid]>key) right=mid-1;//因为mid已经比较过了，所以下一次比较时，去掉这个numbers[mid]
...
void test() {
	...
	printf("%d %d\n", val,arr1[pos1]);
	if (arr1[val] == arr1[pos1]) printf("Correct!\n");
	else printf("Wrong!\n");
}
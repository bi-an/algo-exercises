...
static void PrintByPostorder_Iteratively(TreeNode *root) {
	...
	while (p && p != pre || !S.empty()) {//用pre记录上次访问过的节点
		if (p && p!=pre) {
			S.push(p);
			p = p->left;
		}
...
{
	if (startPreorder == nullptr || endPreorder < startPreorder ||
		startInorder == nullptr || endInorder < startInorder ||
		endPreorder - startPreorder != endInorder - startInorder)
		return nullptr;

	TreeNode *root = new TreeNode(startPreorder[0]);

	if (endPreorder == startPreorder) {//递归终止条件

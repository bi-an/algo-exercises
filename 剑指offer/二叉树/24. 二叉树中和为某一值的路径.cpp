...
public:
	...
	void FindPathCore(TreeNode* root, int expectNumber, vector<int> path, int sum, vector<vector<int>>& res) {
		...
		if (sum == expectNumber) {
			...
			path.push_back(root->val);
			if(root->left) FindPathCore(root->left, expectNumber, path, sum, res);
			if(root->right) FindPathCore(root->right, expectNumber, path, sum, res);
		}
	}
};
...
public:
	...
	void FindPathCore(TreeNode* root, int expectNumber, vector<int> path, int sum, vector<vector<int>>& res) {
		...
		if (sum == expectNumber) {
			if(root->left == nullptr && root->right == nullptr){ //增加判断叶子节点的条件
				path.push_back(root->val);
				res.push_back(path);
			}
			else
...
private:
	...
	void findAllPath(TreeNode* root,int expectNumber, vector<int> &path,vector<vector<int>> &pathList) {//本质是一个先序遍历
		path.push_back(root->val);

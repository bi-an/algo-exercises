...
class Solution
{
 public:
  ...
  bool DoesParentHasChild(TreeNode *parent, TreeNode *child)
  {
    ...
    return parent->val == child->val && DoesParentHasChild(parent->left, child->left) && DoesParentHasChild(parent->right, child->right);
  }
};
...
class Solution
{
 public:
  ...
  {
    if (pRoot2 == nullptr)
      return true; // pRoot2到达叶子节点
    if (pRoot1 == nullptr)
      return false; // pRoot1到达叶子，而pRoot2还没有到达叶子
    if (pRoot1->val != pRoot2->val)
      return false;

...
private:
	...
	BinaryNode* findMin(BinaryNode *t) const { // 似乎不能把定义写在类外,不然找不到返回值类型BinaryNode的定义
		if (t == nullptr)
			return nullptr;
...
template<typename T> const BinarySearchTree<T>& BinarySearchTree<T>::operator=(const BinarySearchTree& rhs) {
	if (this != &rhs) { // 自我复制检测
		makeEmpty();
		root = clone(rhs.root);
	}
...
template<typename T> void BinarySearchTree<T>::makeEmpty(BinaryNode *&t) const {
	if (t != nullptr) {
		makeEmpty(t->left);
		makeEmpty(t->right);
		delete t;
...
// 按层打印，空节点用#表示
template<typename T> void BinarySearchTree<T>::printTree(BinaryNode *t) const {
	if (t == nullptr) return;

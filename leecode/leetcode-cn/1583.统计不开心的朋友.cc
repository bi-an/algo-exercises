...
public:
    int unhappyFriends(int n, vector<vector<int>>& preferences, vector<vector<int>>& pairs) {
        // 1. 预处理，使能够O(1)时间找到对应的亲密程度、配对信息
        // order[i][j]表示第i个朋友与第j个朋友的亲密程度，越低说明越亲密，order其实是preferences的下标
        vector<vector<int>> order(n, vector<int>(n));
        vector<int> match(n); // 记录第i个朋友的配对者
        ...
        for(x=0;x<n;x++) {
            ...
            for(int i=0;i<index;i++) {
                ...
                if(order[u][x] < order[u][v]) {
                    ans++;
                    break; // 这里是对x遍历，只能计算1次，如果x不开心，第二次循环的目的就达到了
                }
            }
        }
